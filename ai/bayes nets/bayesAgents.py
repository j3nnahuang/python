...
def fillObsCPT(bayesNet, gameState):
    ...
    def getQuadrant(x, y):
        vertical_half = gameState.data.layout.width / 2
        horizontal_half = gameState.data.layout.height / 2
        toReturn = None;
        if x < vertical_half and y > horizontal_half:
            toReturn = TOP_LEFT_VAL
        elif x > vertical_half and y > horizontal_half:
            toReturn = TOP_RIGHT_VAL
        elif x < vertical_half and y < horizontal_half:
            toReturn = BOTTOM_LEFT_VAL
        elif x > vertical_half and y < horizontal_half:
            toReturn = BOTTOM_RIGHT_VAL
        return toReturn

    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            _factor = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for quadrant in _factor.getAllPossibleAssignmentDicts():
                p_red, p_blue, p_empty = 0, 0, 0;
                if quadrant[FOOD_HOUSE_VAR] == getQuadrant(obsPos[0], obsPos[1]):
                    p_red, p_blue, p_empty = PROB_FOOD_RED, 1 - PROB_FOOD_RED, 0
                elif quadrant[GHOST_HOUSE_VAR] == getQuadrant(obsPos[0], obsPos[1]):
                    p_red, p_blue, p_empty = PROB_GHOST_RED, 1 - PROB_GHOST_RED, 0
                else: 
                    p_red, p_blue, p_empty = 0, 0, 1

                if quadrant[obsVar] == RED_OBS_VAL:
                    _factor.setProbability(quadrant, p_red)
                elif quadrant[obsVar] == BLUE_OBS_VAL:
                    _factor.setProbability(quadrant, p_blue)
                else:
                    _factor.setProbability(quadrant, p_empty)

            bayesNet.setCPT(obsVar, _factor)
...
class VPIAgent(BayesAgent):
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        prob_and_action = self.getExplorationProbsAndOutcomes(evidence)

        for (probability, action) in prob_and_action:
            expectedValue += probability * max(self.computeEnterValues(action, enterEliminationOrder))
