...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        all_CPTs = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for variable in eliminationOrder:
            toSplit = joinFactorsByVariable(all_CPTs, variable)
            all_CPTs = toSplit[0]
            joined = toSplit[1]

            if len(joined.unconditionedVariables()) > 1:
                factor_to_remove = eliminate(joined, variable)
                all_CPTs += [factor_to_remove]

        return normalize(joinFactors(all_CPTs))
