...
class SigmoidNeuron(object):
    def __init__(self, numFeatures):
        ...
        """YOUR CODE HERE"""
        self.weight = [0.0 for _ in range(numFeatures)]
    ...
    def classify(self, sample):
        ...
        """YOUR CODE HERE"""
        return self._sigmoid(np.dot(sample, self.weight)) > 0.5
    ...
    def train_single(self, sample, alpha, label):
        ...
        """YOUR CODE HERE"""
        def g(f):
            return self.loss(sample, label, f)
        gradient = nu.gradient(g, self.weight)
        for i in range(len(self.weight)):
            self.weight[i] = self.weight[i] - (alpha * gradient[i])
    ...
    def train(self, samples, alpha, labels):
        ...
        for index in range(len(samples)):
            self.train_single(samples[index], alpha, labels[index])
