...
def eval_all(expressions, env):
    ...
    for i in range(len(expressions)-1):
        last_sub_expression = scheme_eval(expressions[i], env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        return target
        # rewrite with env.define method
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        lambda_expr = do_lambda_form(Pair(target.second, expressions.second), env)
        env.bindings[target.first] = lambda_expr
        return target.first
    else:
...
def do_if_form(expressions, env):
    ...
    if len(expressions) == 3:
        ...
        alternative = okay
    if scheme_true(scheme_eval(conditional, env)):
...
def do_and_form(expressions, env):
    ...
    if len(expressions)==0:
    	return True
    for i in range(len(expressions) - 1):
    	last_value = scheme_eval(expressions[i], env)
...
def make_let_frame(bindings, env):
    ...
    keys = [binding.first for binding in bindings]
    for i in range(len(keys)):
        if not scheme_symbolp(keys[i]):
            raise SchemeError
    values = [scheme_eval(binding.second.first, env) for binding in bindings]
    kid = env.make_child_frame(keys, values)
    return kid
...
def check_formals(formals):
    ...
    """
    def wellFormed(formals):
        try:
            if len(formals) <= 1:
                return True
            elif not isinstance(formals.second, Pair):
                return False
            else:
                return wellFormed(formals.second)
        except TypeError as e:
            raise SchemeError
    def allStrings(formals):
        for i in range(len(formals)):
            if not scheme_symbolp(formals[i]):
                return False
        return True
    def notRepeated(formals):
        cache = []
        for i in (range(len(formals))):
            if formals[i] in cache:
                return False
            cache.append(formals[i])
        return True
    if wellFormed(formals) and allStrings(formals) and notRepeated(formals):
        return formals
    elif not wellFormed(formals):
        raise SchemeError("Not Well Formed")
    elif not allStrings(formals):
        raise SchemeError("Not All Strings")
    elif not notRepeated(formals):
        raise SchemeError("Includes Repeats")
    else:
        raise SchemeError
