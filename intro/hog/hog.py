...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
    	opponents_first_digit = opponent_score // 10
    	opponent_second_digit = opponent_score % 10
    	return max(opponents_first_digit, opponent_second_digit) + 1
...
def is_prime(n):
    ...
    numbers_divisible_by = 0
    test_factors = n
    if n == 0 or n == 1:
    	return False
    while test_factors != 0:
    	if n % test_factors == 0:
    		numbers_divisible_by += 1
    	test_factors -= 1
    if numbers_divisible_by > 2:
    	return False
    return True
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
        	...
        	current_turn_score = take_turn(strat0, score1, dice0)
        	score0 += current_turn_score
        else:
        	dice1 = select_dice(score1, score0)
        	strat1 = strategy1(score1, score0)
        	current_turn_score = take_turn(strat1, score0, dice1)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    max_count = 0
    while count < 10:
    	a = make_averaged(roll_dice, 1000)(count,dice)
    	b = make_averaged(roll_dice, 1000)(count + 1, dice)
    	if a > b:
...
def prime_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    otherwise.
    """
    opponents_ten_digit = opponent_score // 10
    opponent_single_digit = opponent_score % 10
    max_score = max(opponent_single_digit, opponents_ten_digit) + 1
    if is_prime (max_score + score + opponent_score) == True:
    	if max_score + score > opponent_score:
    		return 0
    	else:
    		return num_rolls
    if is_prime(score+opponent_score)== False:
    	return num_rolls
    if take_turn (0, opponent_score, dice=six_sided) >= margin:
    	return 0
    return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    if score > 70:
    	return bacon_strategy(score, opponent_score, margin=8, num_rolls=8)
    if score > opponent_score:
    	return prime_strategy(score, opponent_score, margin=8, num_rolls=8)
    if opponent_score-30 > score:
    	return 10
    return 7
