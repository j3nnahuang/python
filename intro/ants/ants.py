    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and not insect.imposter:
                return
            elif insect.container and insect.ant:
                self.ant = insect.ant
            else:
                self.ant = None
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        def helper_nearest_bee(self, new_location):
            if new_location == hive:
                return None
            elif len(new_location.bees) != 0:
                return random_or_none(new_location.bees)
            else:
                return helper_nearest_bee(self, new_location.entrance)
        return helper_nearest_bee(self, self.place)
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        """
        "*** YOUR CODE HERE ***"
        def helper_nearest_bee(self, newLocation):
            if newLocation == hive:
                return None
            elif self.min_range == 0:
                return random_or_none(newLocation.bees)
            else:
                self.min_range -= 1 
                return helper_nearest_bee(self, newLocation.entrance)
        return helper_nearest_bee(self, self.place)
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        def helper_nearest_bee(self, newLocation):
            if newLocation == hive:
                return None
            elif self.test_range > self.max_range:
                return None
            elif len(newLocation.bees) != 0 and self.test_range <= self.max_range:
                return random_or_none(newLocation.bees)
            else:
                self.test_range += 1
                return helper_nearest_bee(self, newLocation.entrance)
...
class WallAnt(Ant):
    """An specialized Ant that does nothing each turn, but is useful because it has a large armor value."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaAnt is watersafe and does not lose its armor when placed in Water"""
    name = 'Scuba'
    watersafe = True
    food_cost = 5
    implemented = True
...
class HungryAnt(ThrowerAnt):
    ...
    def action(self, colony):
        "*** YOUR CODE HERE ***"
        if self.nearest_bee(colony.hive)==None:
            return None
        elif self.digesting == 0:
            self.eat_bee(self.nearest_bee(colony.hive))
        else: 
            self.digesting -= 1
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.index > 1:
            ...
            def damage_doubler(somePlace):
                if somePlace.ant and somePlace.ant.alreadyIncreased==False:
                    somePlace.ant.alreadyIncreased = True
                    somePlace.ant.damage *= 2
                    if somePlace.ant.container:
                        if somePlace.ant.ant:
                            somePlace.ant.ant.alreadyIncreased = True
                            somePlace.ant.ant.damage *= 2

            if self.place.ant.container:
                if self.place.ant != None and self.place.ant.alreadyIncreased == False:
                    self.place.ant.damage *= 2
                    self.place.ant.alreadyIncreased = True

            left_side = self.place.exit
            while left_side:
                damage_doubler(left_side)
                left_side = left_side.exit

            right_side = self.place.entrance
            while right_side:
                damage_doubler(right_side)
                right_side = right_side.entrance
