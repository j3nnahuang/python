...
def group_by_centroid(restaurants, centroids):
    ...
    """
    closest_list = [[find_closest(restaurant_location(x), centroids), x] for x in restaurants]
    return group_by_first(closest_list)

def find_centroid(restaurants):
    """Return the centroid of the locations of RESTAURANTS."""
    avg_latitude = mean([restaurant_location(x)[0] for x in restaurants])
    avg_longitude = mean([restaurant_location(y)[1] for y in restaurants])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    mean_y = mean(ys)
    print(mean_y)
    S_xx = sum([pow((i - mean_x), 2) for i in xs])
    print ("S_xx is ", S_xx)
    S_yy = sum([pow((i - mean_y), 2) for i in ys])
    print ("S_yy is ", S_yy)
   
    #S_xy = sum([(i - mean_x)*(j - mean_y) for i in xs for j in ys if xs.index(i) == ys.index(j)])
    S_xy = 0
    for i in range(0, len(xs)):
        S_xy += (xs[i] - mean_x)*(ys[i] - mean_y)
    print(S_xy)
    b, a, r_squared = (S_xy/S_xx), mean_y - (S_xy/S_xx)*mean_x, pow(S_xy, 2)/(S_xx*S_yy)  # REPLACE THIS LINE WITH YOUR SOLUTION
    print (b)
    print(a)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    predictor_dictionary = {find_predictor(user, reviewed, i)[0]: find_predictor(user, reviewed, i)[1] for i in feature_fns}
    return max(predictor_dictionary, key=lambda x: predictor_dictionary[x])
...
def rate_all(user, restaurants, feature_functions):
    ...
    has_been_reviewed = user_reviewed_restaurants(user, restaurants)
    for i in restaurants.keys():
        if i in has_been_reviewed:
            restaurants[i] = user_rating(user, i)
        else:
            restaurants[i] = predictor(restaurants[i])
    return restaurants
...
def search(query, restaurants):
    ...
    for i in restaurants:
        if query in restaurant_categories(i):
            restaurants_with_query.append(i)
